...
def agent_to_object():
	...
	location = robots[int(index[1])-1]["location"].split(", ")
...
class Robot():
	...
	def declare_end_callback(self, req):
		...
		self.resp1 = DeclareEndResponse()
		for t in self.real_robot_schedule:
			self.resp1.schedule.append(t.to_msg())
	...
	def is_Valid(self, task):
		...
		# tasks earliest possible start time and latest possible finish time.
		...
		if not self.real_robot_schedule:
			if not self.robot_schedule:
				schedule = []
				schedule.append(task)
				if (self.get_Schedule_Time(schedule) + task.earliest_start_time) <= task.latest_finish_time:
					result = True
				else:
					return False
			else:
				schedule = self.robot_schedule[:]
				schedule.append(task)
				if self.get_Schedule_Time(schedule) >= task.earliest_start_time and self.get_Schedule_Time(schedule) <= task.latest_finish_time:
					result = True
				else:
					return False
		else:
			if not self.robot_schedule:
				schedule = self.real_robot_schedule[:]
				schedule.append(task)
				if self.get_Schedule_Time(schedule) >= task.earliest_start_time and self.get_Schedule_Time(schedule) <= task.latest_finish_time:
					result = True
				else:
